...
P10 qualification requires manifest/cwd fail-closed behavior, literal argv handling, missing-executable and failure classification, stdout and timeout bounds, no repair authority, no-progress and state-lock behavior, capability honesty, tracked-byte identity invalidation, explicit noninteractive runtime environment, environment-bound command identity, and process-vs-harness outcome separation. Local outcomes are explicitly `LOCAL_QUALIFIED`, `LOCAL_FAILED`, or `LOCAL_INCOMPLETE`; they never become `CI_PASS` or `CERTIFIED`. The bounded-evidence campaign layer can additionally freeze an external controller budget, internal batch timeout, and minimum headroom. Plans without headroom fail before execution; controller timeout and completed-over-budget receipts are `INCOMPLETE`, never product failures, and completed sibling receipts remain reusable.
...
The new `refactor-locality` contract records transparent pre/post dimensions with no composite score, binds measurements to repository/source/configuration identities, rejects incomplete or incomparable evidence, and distinguishes cohesive authority from justified decomposition, locality regression, and insufficient evidence. Size-only evidence cannot justify a split. Every newly introduced symbol/layer must bind an allowed structural value and its evidence identity. The qualification explicitly rejects pass-through wrappers/shims/delegates and same-file helper extraction without evidence-bound semantic value when those structures exist only to lower measured complexity. `refactor-focus` no longer recommends wrappers/splits from size/dependency counts and no longer ranks by an opaque risk score; it ranks investigations by visible dimensions and requires locality evidence before decomposition. BP2 remains behavior-preservation evidence and now explicitly requires locality remeasurement rather than implying architectural improvement from green tests or debt reduction. The follow-on provider-binding closure makes Hashmarks structural-locality packets the independent authority for caller counts, forwarding shape, navigation/file/context dimensions, verifier paths, ambiguity, and repository/source identities. Independent authority additionally requires an agentsCookbook-observed bounded Hashmarks execution receipt bound to the exact target, bounds, producer implementation identity, packet identity, and unchanged tracked workspace bytes. Hand-authored locality snapshots, packet-only JSON, and caller-supplied saved receipt bundles cannot justify a decomposition; the live `observe-hashmarks` path owns provider-execution promotion.

The debt-closure contract now consumes that locality result instead of treating remeasurement as advisory. Analyzer/Ruff reduction cannot become `VERIFIED` without an identity-valid, independently Hashmarks-backed post-edit locality snapshot and matching locality decision. A cohesive in-place cleanup can close when locality is preserved or improved and no new structure was introduced. If the edit introduced helpers/layers, closure requires `DECOMPOSITION_JUSTIFIED`; evidence-bound semantic value alone is not enough without decomposition evidence and any required bounded-locality tradeoff receipt. Newly introduced symbols with zero or one **observed exact caller** are surfaced as limited-observed-reuse review evidence, never as proof of global single-use. `shared_reuse` requires at least two exact non-verifier callers so direct tests cannot inflate reuse evidence.
