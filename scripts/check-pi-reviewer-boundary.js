...
async function main() {
  ...
  if (!same(core.PI_REVIEWER_AGENTS, expectedReviewers)) {
    fail(`reviewers=${JSON.stringify(core.PI_REVIEWER_AGENTS)}`);
  }
  if (!same(core.PI_REVIEWER_BASE_TOOLS, expectedBaseTools)) {
    fail(`base_tools=${JSON.stringify(core.PI_REVIEWER_BASE_TOOLS)}`);
  }
  if (!same(core.reviewerAllowedTools(), expectedBaseTools)) {
    fail('normal_mode_tool_boundary_mismatch');
  }
  if (!same(core.reviewerAllowedTools({ artifactEnabled: true }), [...expectedBaseTools, 'review_artifact'])) {
    fail('artifact_mode_tool_boundary_mismatch');
  }

  for (const denied of ['bash', 'powershell', 'edit', 'write', 'subagent', 'task', 'review_artifact_read']) {
    if (core.reviewerToolAllowed(denied, { artifactEnabled: true })) {
      fail(`dangerous_tool_allowed=${denied}`);
    }
  }
  ...
  if (core.activeReviewerName({ PI_OPEN_AGENTS_DEPTH: '0', PI_OPEN_AGENTS_NAME: expectedReviewers[0] })) {
    fail('primary_process_misclassified_as_reviewer_child');
  }
  if (core.activeReviewerName({ PI_OPEN_AGENTS_DEPTH: '1', PI_OPEN_AGENTS_NAME: 'unknown-agent' })) {
    fail('unknown_agent_misclassified_as_reviewer');
  }
  ...
  process.stdout.write('CHECK name=pi_reviewer_runtime_boundary status=pass reviewers=9 normal_tools=4 artifact_tools=5 defense_in_depth=true\n');
}
