...
- `call-chain-collapse-review` targets no-value hops regardless of tool ownership; `native-tool-authority-review` targets wrappers or layers that recreate semantics already owned by an external/native tool. `entrypoint-authority-review` targets multiple launch/configuration owners for one conceptual action even when every individual path is short. `dependency-surface-review` targets oversized inputs/contexts even when the call depth is reasonable.
