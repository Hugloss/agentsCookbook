...
description: Designs minimal decisive validation with observable checks, failure cases, acceptance criteria, and recovery proof.
license: MIT
---
...
> **Completion must be observable; proposed checks must prove the behavior that matters.**
...
Map each important invariant or user-visible behavior to the smallest decisive check. Mark commands/paths as assumptions until verified.
...
Return `# Validation Design Report` with verdict, strategy, automated checks, manual checks, acceptance criteria, failure/edge scenarios, rollback verification, missing repo facts, and concrete fixes.
